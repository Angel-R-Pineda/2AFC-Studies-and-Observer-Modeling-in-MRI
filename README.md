...
## Using the Sparse Difference of Gaussians (SDOG) observer model to estimate AUC

In the same folder there is also code to implement the SDOG model observer.  The SDOG observer models human perfomance in a dectection task using three channels in the frequency domain inspired by the human visual system.  The SDOG model used in this work comes from Abbey, et. al.
https://www.ncbi.nlm.nih.gov/pmc/articles/PMC2943344/

There are two MATLAB files used in the implementation of the SDOG observer.
